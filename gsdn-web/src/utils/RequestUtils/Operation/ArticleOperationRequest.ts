...
class ArticleOperationRequest{
    ...
    private static _updateArticle(title:string | undefined, content:string | undefined, ispublic:1 | -1 | undefined,isMarkdown:boolean,publicsort:string,sortname:string,tags:string[],description:string,file:File,articleid:string): Promise<object> {
        return new Promise(async resolve => {
            try {
                let url = isMarkdown ? 'Edit/updatemd' : 'Edit/Update'
                Promise.all([
                    this.updateArticleContent(url,title,content,ispublic,articleid),
                    this.modifyArticleTags(articleid,tags),
    ...
    private static doArticleTagsOperation(url:string,tags:string[],articleid:string):Promise<object>{
        if (tags.length < 1) return Promise.resolve({Ok:true})
        else return new Promise(async (resolve,reject) => {
            try {
                let tagStr = JSON.stringify(tags);
                let result = await doRequest({url,data:{token:CustomStorage.getAccount().Token,tags:tagStr.substring(1,tagStr.length - 1),articleid},method:'GET'})
                resolve({Ok:result?.Ok})
            }catch (e){
                resolve({Ok:false})
            }
    ...
    private static addArticleTags(articleid: string, tags: string[]): Promise<object> {
        return this.doArticleTagsOperation('tags/add',tags,articleid);
    }
    private static modifyArticleTags(articleid:string,tags:string[]):Promise<object>{
        return this.doArticleTagsOperation('tags/update',tags,articleid);
    }
    /**
     * @deprecated
     */
